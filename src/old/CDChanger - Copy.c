...
TODO: 
- If you have 2 CDROMs does the volume name check work?
- Split the SCSI code out
...
int main(int argc, char **argv)
{
   ...
   if (BlueSCSI_InitDevice() < 0)
   {
      MessageBox("Error sending inquiry to device\n");
      goto exit;
   }
   if (!scsi_isBlueSCSI)
   {
      MessageBox("Not a BlueSCSI CDROM!\n");
      goto exit;
   }
#endif
   ...
   {
      MessageBox("Could not open window\n");
   }
...
int BlueSCSI_InitDevice()
{
   ...
   {
      int scsi_removable = (scsi_data[1] & 0x80) ? 1 : 0;
      //scsi_isCD = (scsi_data[0] & 0x1F) ? 0x05 : 0x00;
      scsi_isBlueSCSI = (scsi_removable == 1) && Strnicmp("BlueSCSI", &scsi_data[8], 8) == 0;
   }
   return 0;
}
